...
def sanitize_name(path):
    ...
    # Replace trailing spaces in each part with underscores
    sanitized_parts = [part.rstrip().replace(' ', '_') for part in parts]
...
def extract_zip_files(zip_folder, final_destination_base):
    ...
    for zip_file in os.listdir(zip_folder):
        if zip_file.endswith('.zip'):  # Process ZIP files only
            zip_path = normalize_path(os.path.join(zip_folder, zip_file))
            final_destination = normalize_path(os.path.join(final_destination_base, os.path.splitext(zip_file)[0]))
            os.makedirs(final_destination, exist_ok=True)

            temp_dir = tempfile.mkdtemp()

            try:
                #print(f"Extracting {zip_file} to temporary directory {temp_dir}...")
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    for file in zip_ref.namelist():
                        sanitized_name = sanitize_name(file)  # Sanitize file/folder names
                        target_path = normalize_path(os.path.join(temp_dir, sanitized_name))
                ...
                for item in os.listdir(temp_dir):
                    ...
                    destination1 = os.path.join(final_destination, item)
                    destination = os.path.dirname(destination1)
                    #print(source, destination, item, destination1)
                    if os.path.exists(destination):
                        #print(f"Overwriting existing {destination}...")
                        if os.path.isdir(destination):
                            shutil.rmtree(destination)
                        else:
                            os.remove(destination)
                    shutil.move(source, destination)
                print(f"Files from {zip_file} moved to {final_destination} successfully!")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Download materials for each course
